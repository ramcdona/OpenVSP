...
PropGeom::PropGeom(Aircraft* aptr) : Geom(aptr)
{
    ...
    char name[255];
    sprintf_s( name, "Prop_%d", geomCnt ); 
    geomCnt++;
...
//==== Write Cross Section =====//
//void PropGeom::write_xsec( int index, float mat[4][4], FILE* file_id, double scale_factor )
//{
//  vec3d temp;
//
...
void PropGeom::dump_xsec_file(int geom_no, FILE* dump_file)
{
  ...
  int num_xsecs;
  double rot = 0.0;
  //Af* foil = sectVec[0].foil;
  vec3d pnt;
  vector< Section > sectVec;              // One Blade - Section Vec

  int nblades = bladeVec.size();
  vec3d origin = vec3d(0., 0., 0.).transform(model_mat);
  // from update_bbox
  for (int nb = 0; nb < nblades; nb++) // nblades
  {
      //==== Check All Xsecs ====//
      num_pnts = bladeVec[nb].get_num_pnts();
      num_xsecs = bladeVec[nb].get_num_xsecs();

	  for ( int i = 0 ; i < num_xsecs ; i++ )
      //for ( int i = 0 ; i < (int)sectVec.size() ; i++ ) // number of blade sections
      {
          //twist  = sectVec[i].twist;
          fprintf(dump_file, "\n");
          fprintf(dump_file, "%s \n", (char*) getName());
          fprintf(dump_file, " GROUP NUMBER      = %d\n", geom_no);
          fprintf(dump_file, " TYPE              = 2\n");
          fprintf(dump_file, " CROSS SECTIONS    = %d\n", num_xsecs);
          fprintf(dump_file, " PTS/CROSS SECTION = %d\n", num_pnts);
          fprintf(dump_file, " NBLADES           = %d\n", nblades);
          fprintf(dump_file, " IBLADE            = %d\n", nb);
          fprintf(dump_file, " ISECTION          = %d\n", i);
          fprintf(dump_file, " ORIGIN            = %25.17e %25.17e %25.17e\n", origin.x(), origin.y(), origin.z());
          //fprintf(dump_file, " TWIST             = %12.8f\n", twist);

          //chord  = sectVec[i].chord;
          //loc    = sectVec[i].x_off;
          //offset = sectVec[i].y_off;
          //foil   = sectVec[i].foil;

          //bladeVec[nb].write_xsec(i, model_mat, dump_file);
          for ( int j = 0 ; j < num_pnts ; j++ )
          {
            pnt = bladeVec[nb].get_pnt(i, j).transform(model_mat);
            // secondary blade geometry is not correct as they are not rotated around the
            // axis of rotation by the rotation angle
